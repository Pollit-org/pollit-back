...
{
    public UserCreatedEvent(User user) : this(user.Id, user.UserName, user.Email, user.IsEmailVerified) { }
    
    public UserCreatedEvent(UserId userId, UserName userName, Email userEmail, bool userEmailIsVerified)
    {
        UserId = userId;
        UserName = userName;
        UserEmail = userEmail;
